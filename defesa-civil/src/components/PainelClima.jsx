...
export default function PainelClima({ cidade }) {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=temperature_2m,relative_humidity_2m,precipitation,weather_code,wind_speed_10m,apparent_temperature&forecast_days=1&timezone=America%2FSao_Paulo`
    )
      .then(r => r.json())
      .then(data => {
