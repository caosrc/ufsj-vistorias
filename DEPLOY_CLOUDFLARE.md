...
> O `schema.sql` já inclui todas as tabelas: `ocorrencias`, `vistorias`, `imoveis`, `areas_risco` e `vistoria_monitoramento` (seção Monitoramento).

**Se o banco já existia e você está migrando**, execute adicionalmente:
```bash
npx wrangler d1 execute geovistorias --remote --command="CREATE TABLE IF NOT EXISTS vistoria_monitoramento (id TEXT PRIMARY KEY, proprietario TEXT NOT NULL, cidade TEXT, endereco TEXT, celular TEXT, latitude REAL, longitude REAL, nome TEXT, fotos_anomalias TEXT DEFAULT '[]', relatorios_salvos TEXT DEFAULT '[]', created_at TEXT DEFAULT (datetime('now')), updated_at TEXT DEFAULT (datetime('now')))"
```
...
| `AUTH_USERS_JSON`         | Mesmo valor do secret Replit — ex: `[{"email":"...","senha":"..."}]` |
